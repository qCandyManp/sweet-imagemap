...
namespace Qcandymanp\SweetImagemap\Controller\Wizard;
...
class ImageMapController
{
    ...
    public function __construct(StandaloneView $templateView = null)
    {
        if (!$templateView) {
            ...
            $templateView->setLayoutRootPaths([GeneralUtility::getFileAbsFileName('EXT:sweet_imagemap/Resources/Private/Layouts/')]);
            $templateView->setPartialRootPaths([GeneralUtility::getFileAbsFileName('EXT:sweet_imagemap/Resources/Private/Partials/ImageMap/')]);
            $templateView->setTemplatePathAndFilename(GeneralUtility::getFileAbsFileName('EXT:sweet_imagemap/Resources/Private/Templates/ImageMap/ImageMapWizard.html'));
        }
    ...
    {
        $token = GeneralUtility::hmac($request->getParsedBody()['arguments'], 'ajax_wizard_imagemap');
